...
function M.eliminate(m, am, sr, sc) --start row , stat col
	...
	if M.iszero(first) then
		for r = sr + 1, m.r do
			local cur = m:get(r, sc)
			if not M.iszero(cur) then
				M.swaprow(m, sr, r)
				M.swaprow(am, sr, r)
				break
			end
...
function M.addrow(m, am, r1, r2, c)
	...
	for i = 1, m.c do
		if not M.iszero(row1[i]) then
			row2[i] = row1[i] * factor + (row2[i] or 0)
		end

		if not M.iszero(amrow1[i]) then
